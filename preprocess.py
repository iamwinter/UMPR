...
def process_dataset(json_path, select_cols, train_rate, save_dir):
    ...
    train.to_csv(os.path.join(save_dir, 'train.csv'), index=False, header=False)
    valid.to_csv(os.path.join(save_dir, 'valid.csv'), index=False, header=False)
    test.to_csv(os.path.join(save_dir, 'test.csv'), index=False, header=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', dest='data_path',
                        default='data/amazonCSJ/reviews_Clothing_Shoes_and_Jewelry_5.json.gz',
